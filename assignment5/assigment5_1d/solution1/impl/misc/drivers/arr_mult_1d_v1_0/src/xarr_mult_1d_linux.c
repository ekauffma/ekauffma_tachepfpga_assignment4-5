...
typedef struct {
    ...
    XArr_mult_1d_uio_map maps[ MAX_UIO_MAPS ];
} XArr_mult_1d_uio_info;

/***************** Variable Definitions **************************************/
static XArr_mult_1d_uio_info uio_info;
...
static int uio_info_read_name(XArr_mult_1d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XArr_mult_1d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XArr_mult_1d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XArr_mult_1d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XArr_mult_1d_Initialize(XArr_mult_1d *InstancePtr, const char* InstanceName) {
	XArr_mult_1d_uio_info *InfoPtr = &uio_info;
